...
// diagnostics_watchdog_refresh removed — was dead code with zero callers
...
void diagnostics_print_errors(void) {
    ...
    /* Brown-out first: it is latched, it refuses every motor start, and it is
     * the only entry here that cannot be cleared by anything short of a power
     * cycle — so the operator needs it at the top, not buried under timeouts. */
    if (g_brownout_latched) {
        uart_puts("⚠ SUPPLY BROWN-OUT detected since boot\r\n");
        uart_puts("  VDD fell through the PVD threshold (~2.5 V).\r\n");
        uart_puts("  Motor start is REFUSED until the machine is power cycled.\r\n");
        any_errors = true;
    }
    ...
    // MCB communication health
    /* REVIEW FIX: guard the subtraction and the division. Both operands are
     * uint32_t, so success > total (however it arose) underflows to a huge
     * failure count, and the divisor below is mcb_total_queries — zero for as
     * long as diagnostics_mcb_comm() had no callers, which was its entire life
     * until now. Unreachable by luck, not by construction. */
    uint32_t mcb_failures = (diag.mcb_total_queries > diag.mcb_comm_success)
                                ? (diag.mcb_total_queries - diag.mcb_comm_success)
                                : 0;
    if (mcb_failures > 0 && diag.mcb_total_queries > 0) {
        uart_puts("⚠ MCB Communication:\r\n");
        uart_puts("  Failed: ");
