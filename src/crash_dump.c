...
/* Address and size come from eeprom_layout.h so there is one authority. The
 * literal 0xDC that used to live here disagreed with the compiled struct and
 * with the layout header, and the settings block had grown across it. */
#define CRASH_DUMP_EEPROM_ADDR  EE_CRASH_BASE
...
void crash_dump_init(void) {
    ...
    crash_dump_t dump;
    bool read_ok = (eeprom_read(CRASH_DUMP_EEPROM_ADDR, (uint8_t*)&dump, sizeof(dump)) == EEPROM_OK);
...
void crash_dump_log(crash_type_t type, uint32_t pc, uint32_t lr, uint32_t psr, const char* task_name) {
    ...
    dump.type = (uint8_t)type;
    dump.pc = pc;
    dump.lr = lr;
    (void)psr;   /* no longer persisted — see crash_dump.h */
    ...
    /* Every caller is a fault hook that never returns, so the bus lock is
     * abandoned rather than waited on — see eeprom_enter_crash_context(). */
    extern void eeprom_enter_crash_context(void);
    eeprom_enter_crash_context();
