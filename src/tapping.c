...
static tapping_settings_t tap_settings = {
    ...
    .pedal_enabled = 0,
    .speed_rpm = 200,
    .depth_completion_action = COMPLETION_STOP,
...
void tapping_set_depth_completion_action(uint8_t action) {
    tap_settings.depth_completion_action = action;
}
...
// AUDIT FIX (HIGH task_tapping.c:379 + MEDIUM menu.c:545): setters that used
// to be missing, so the corresponding menu edits and persisted values were
// silently discarded (peck completion action / reverse_out) and boot-time
// settings sync had no way to push these fields to the runtime store.
...
void tapping_set_clutch_action(uint8_t action) {
    tap_settings.clutch_action = action;
}

void tapping_set_peck_completion_action(uint8_t action) {
    tap_settings.peck_completion_action = action;
}

void tapping_set_quill_completion_action(uint8_t action) {
    tap_settings.quill_completion_action = action;
}

void tapping_set_load_completion_action(uint8_t action) {
    tap_settings.load_completion_action = action;
}

void tapping_set_load_slip_completion_action(uint8_t action) {
    tap_settings.load_slip_completion_action = action;
}

void tapping_set_peck_reverse_out_ms(uint16_t ms) {
    if (ms > 10000) ms = 10000;
    tap_settings.peck_reverse_out_ms = ms;
}

void tapping_set_pedal_action(uint8_t action) {
