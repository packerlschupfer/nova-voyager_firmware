...
/* AUDIT FIX (HIGH, events.c:441): sticky "was engaged/open at least once since
 * the last read" latches. The event handlers used to branch on the CURRENT pin
 * level, but task_ui.c:202 (priority 2) refreshes g_state.guard_closed from the
 * live pin every 2 ms while task_main runs at priority 1 — so a guard bumped
 * open and shut before its event was dequeued matched NEITHER branch: no queue
 * purge, no CMD_MOTOR_STOP, no spindle hold, and no motor_hardware_enable() to
 * undo the ISR's PD4 drop. The spindle coasted unbraked while g_state.state
 * still said APP_STATE_DRILLING. The edge is what matters, so the edge is what
 * gets latched. */
static volatile bool estop_engaged_latch = false;
static volatile bool guard_opened_latch = false;
...
#ifdef BUILD_DEBUG
uint32_t encoder_get_isr_count(void) { return isr_count; }
int16_t encoder_get_position(void) { return position; }
#endif
...
// AUDIT FIX (HIGH, encoder.c:260): the EXTI ISRs debounce by "≥50 ms since
// last accepted edge", which does NOT filter break-bounce on release — any
// press held longer than 50 ms is followed by chattering falling edges when
// the contact opens, and the first one is accepted as a fresh press. For
// START that meant releasing a stop-press restarted the spindle.
//
// Fix: requalify the ISR-set event against the pin level at read time.
// task_ui polls at 2 ms; by the time it consumes an event, a genuine press
// still holds the pin LOW while a release-bounce phantom is HIGH again.
// Reading GPIOx->IDR here is cheap and unambiguous — pin is active-low, so
// LOW = pressed, HIGH = filter.

// Check and clear button events (called from task_ui)
bool encoder_f1_clicked(void) {
    if (btn_f1_event) {
        btn_f1_event = false;
        if (GPIOC->IDR & (1 << 10)) return false;  // PC10 HIGH = not pressed → filter
        return true;
    }
    return false;
}
...
bool encoder_f2_clicked(void) {
    if (btn_f2_event) {
        btn_f2_event = false;
        if (GPIOC->IDR & (1 << 11)) return false;
        return true;
    }
    return false;
}
...
bool encoder_f3_clicked(void) {
    if (btn_f3_event) {
        btn_f3_event = false;
        if (GPIOC->IDR & (1 << 12)) return false;
        return true;
    }
    return false;
}
...
bool encoder_start_clicked(void) {
    if (btn_start_event) {
        ...
        if (GPIOA->IDR & (1 << 15)) return false;  // PA15 HIGH = not pressed → filter
        return true;
    }
    return false;
}
...
void EXTI0_IRQHandler(void) {
    ...
    if (estop_state) {
        ...
        estop_engaged_latch = true;     // survives a release before the handler runs
    }
    // Note: Motor enable will be restored by main task after E-Stop cleared

    estop_changed = true;

    /* REVIEW FIX: this hand-rolled send discarded xQueueSendFromISR()'s
     * result, so a full event queue dropped the E-Stop notification without
     * even bumping the overflow counter that exists for exactly this. The
     * hardware cutoff above has already dropped PD4, so the spindle is dead
     * either way — but the state update, the screen and the diagnostic count
     * all went missing silently. SEND_EVENT_ISR() counts it. */
    SEND_EVENT_ISR(EVT_BTN_ESTOP);
}
...
void EXTI2_IRQHandler(void) {
    ...
    /* SAFETY: guard opened -> disable the motor immediately.
     *
     * REVIEW FIX (HIGH): this ignored sensor.guard_check_enabled, which
     * task_ui.c, display.c and events.c all honour. So switching Guard Check
     * OFF — what you do on a machine with no guard switch, or a broken one —
     * still let a single chatter pulse drop PD4, abort the cut, purge the motor
     * queue and paint "GUARD OPENED!" for 30 s. A setting that disables the
     * guard interlock has to disable all of it, or it disables none of it.
     *
     * settings_get() returns a pointer to a plain struct; reading one bool from
     * an ISR is a single aligned load. */
    const settings_t* gs = settings_get();
    const bool guard_check_on = (gs == NULL) || gs->sensor.guard_check_enabled;

    if (guard_state && guard_check_on) {
        // Guard opened - disable motor hardware NOW
        GPIOD->BSRR = (1 << (4 + 16));  // BR4 = reset PD4 (motor enable)
        guard_opened_latch = true;      // survives a close before the handler runs
    }

    guard_changed = true;

    SEND_EVENT_ISR(EVT_BTN_GUARD);   /* same reasoning as the E-Stop ISR */
}
...
/* Read-and-clear. True if the guard has been open at any point since the last
 * call, even if it is closed again now. Single reader (task_main's
 * handle_btn_guard); the ISR only ever sets, so no lost-update race. */
bool encoder_guard_opened_since(void) {
    if (guard_opened_latch) {
        guard_opened_latch = false;
        return true;
    }
    return false;
}

/* Read-and-clear. See encoder_guard_opened_since(). */
bool encoder_estop_engaged_since(void) {
    if (estop_engaged_latch) {
        estop_engaged_latch = false;
        return true;
    }
    return false;
}
