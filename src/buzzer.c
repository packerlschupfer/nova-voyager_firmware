...
static void delay_ms(uint16_t ms) {
    /* Inner loop is ~8 cycles, so one millisecond is SYSCLK/8 iterations.
     * REVIEW FIX: this was a fixed 15000, i.e. 1 ms only at 120 MHz — the
     * env:release and env:debug builds run at 72 MHz (config.h defaults
     * USE_120MHZ to 0) and every beep came out 67 % long. The prescaler beside
     * it was already conditional; this was not. */
    const uint32_t iters_per_ms = SYSCLK_FREQ / 8000u;
    for (uint16_t i = 0; i < ms; i++) {
        for (volatile uint32_t j = 0; j < iters_per_ms; j++) {
            __NOP();
        }
    }
}
...
// buzzer_update, buzzer_stop, buzzer_is_playing, buzzer_set_enabled,
// buzzer_is_enabled removed — zero callers in production code
