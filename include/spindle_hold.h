...
/**
 * @brief spindle_hold_update() with the UART refresh suppressible.
 *
 * Pass allow_uart = false while an MCB parameter-write envelope is claimed
 * (motor_scan_mode): the safety timeout still runs, the VR/CL/VS refresh is
 * skipped so it cannot splice into another task's command sequence.
 */
