...
/* Energizing align entry points. Both refuse when safety_can_start_motor()
 * says no (E-Stop, open guard, fault, clock fault, flash write in progress).
 * @return false if refused — report safety_refusal_reason() to the operator. */
...
/** @brief True when an ALIGN session has outlived ALIGN_SESSION_TIMEOUT_MS. */
...
/**
 * @brief Read the GR sensor byte using the COMMAND frame format.
 * @return 0..255 on success, -1 if the MCB did not answer.
 * @note GR does NOT answer a query frame; see the implementation.
 */
int16_t motor_read_gr(void);

/** @brief As motor_read_gr(), but also reports the raw bytes received. */
