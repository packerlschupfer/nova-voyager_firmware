...
/**
 * @brief Look at the next queued RX byte without consuming it
 *
 * check_serial_commands() dispatches a command handler from inside its own
 * drain loop, so while a handler runs the RX ring may already hold the *next*
 * pipelined command (a scripted driver sending several lines at once, or the
 * LF of a CRLF terminator). A long-running handler that wants an abort key
 * must therefore peek rather than getc, or it eats those bytes.
 *
 * @return Next byte if available, -1 if buffer empty
 */
int uart_peek_nonblocking(void);

/**
 * @brief UART output usable from fault/exception context
 *
 * No FreeRTOS calls and no mutex — safe from HardFault, where uart_puts()
 * is not. Use these in any path where the system is already failing.
 */
