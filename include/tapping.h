...
/* Menu row "Cycles" offers 0-99, and 0 means "peck until target depth".
 * settings.peck_cycles is a uint8_t, so the console TAPPECK command must
 * range-check against this: an unchecked 256 truncated to 0 and turned a typo
 * into an unbounded peck cycle. */
#define TAP_PECK_CYCLES_MAX         99
 ...
 * @param action completion_action_t (STOP, REVERSE_OUT or REVERSE_TIMED)
 */
