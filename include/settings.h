...
/* Layout version of settings_t, as stored in the flash mirror.
 *
 * Reset to 1 on 2026-08-30: v0.1.0 is the first public release, the project
 * carries no back compatibility by decision, and the counter had only reached
 * 3 through pre-release churn. Starting from 1 makes the number mean
 * "revisions of the shipped layout" rather than "revisions of anything we ever
 * tried".
 *
 * It still has to be BUMPED on any settings_t layout change. `magic` and
 * `version` sit at a fixed offset, so a stale mirror passes both of those
 * gates and would be caught only by the CRC — relying on a checksum to notice
 * a layout change is luck, not detection. There is no migration: a mismatch
 * falls back to defaults, deliberately (see settings_init). */
...
typedef struct {
    ...
    /* All five completion actions are consumed by task_tapping now, and their
     * menu rows are back. The legacy two-value depth_action is gone: it was a
     * second store for the same decision as depth_completion_action, one value
     * short (no REVERSE_TIMED), and its REVERSE branch fell through to an
     * open-ended reverse. Its persisted byte carries depth_completion_action
     * instead — the old encoding maps onto the new one exactly, 0=STOP and
     * 1=REVERSE_OUT, so a stored value keeps its meaning. */
    uint8_t  depth_completion_action;   // completion_action_t: action at target depth
...
// Display settings
/* Only `units` survives here. brightness, contrast, main_div, sub_div,
 * main_round, sub_round, load_display and speed_round were removed on
 * 2026-08-30: nothing read any of them, no menu row or console key set them,
 * their setters had no callers, and the hardware cannot honour the first two
 * anyway — the ST7920 is wired as RS/RW/E on PB0-2 plus 8-bit data on PA0-7
 * with no backlight pin, and its contrast is the panel's Vo trimpot. They were
 * also self-inconsistent: set_defaults() wrote brightness = 100 while the
 * setter clamped to 63, so the default was unreachable by any path that could
 * run. Removing them takes ~18 bytes out of settings_t and, more usefully,
 * stops them being cited as settings that "cannot fit" in the EEPROM block. */
typedef struct {
...
typedef struct {
    ...
    // OEM-style detectors (see docs/MOTOR_PROTOCOL.md "Three independent monitors")
    uint8_t  step_thresh;       // KR step jam threshold (% delta vs prev sample, e.g. 20)
    bool     low_load_detect;   // Low-load (belt break / tool detach) detector ON/OFF
    uint8_t  low_load_thresh;   // Low-load floor (%, e.g. 5) — KR below this + CV<25 triggers
    ...
    uint8_t  overload_threshold; // MCB overload trip point (LD register, 10-100%, default 50)
} sensor_settings_t;
...
/**
 * @brief Outcome of settings_save().
 *
 * REVIEW FIX: it used to return a bare bool, and the callers' failure text
 * assumed the only way to fail was the deferred flash mirror. It has three
 * false paths, and they need different words in front of the operator: a
 * deferred mirror means "stop the spindle and save again", a write error means
 * "the hardware did not take it", and on a flash-only unit with the motor
 * running NOTHING was written at all while RAM already holds the new values.
 */
typedef enum {
    SETTINGS_SAVE_OK = 0,      /**< everything that had to be written, was */
    SETTINGS_SAVE_DEFERRED,    /**< EEPROM written; flash mirror needs a stopped spindle */
    SETTINGS_SAVE_BLOCKED,     /**< flash-only unit, motor running: nothing written */
    SETTINGS_SAVE_ERROR,       /**< a write was attempted and failed */
} settings_save_result_t;

/**
 * @brief Persist the current settings.
 *
 * @return SETTINGS_SAVE_OK when everything that had to be written was;
 *         otherwise which of the three failure modes occurred — the caller is
 *         expected to tell the operator, because they need different actions.
 *
 * Note SETTINGS_SAVE_OK is 0: this is an outcome code, not a truthiness test.
 */
...
/**
 * @brief Push persisted tapping fields (settings_get()->tapping) into the
 *        runtime tap_settings store in tapping.c. Must be called at boot
 *        after settings_init() — otherwise the trigger enables, thresholds,
 *        and peck timings stay at their static zero-defaults regardless of
 *        what the user configured in the menu. Also called from
 *        menu_apply_settings() to keep the two stores in sync.
 */
...
/**
 * @brief Persist the live spindle speed once the operator stops adjusting it.
 *
 * Restores the original firmware's behaviour of remembering the last-used
 * speed across a power cycle. Call periodically (task_main); it debounces
 * internally and writes at most two EEPROM bytes per adjustment.
 *
...
/**
 * @brief Record that the OPERATOR selected a spindle speed.
 *
 * Call from the paths that represent a human choosing: the encoder, favourite
 * recall, and the console SPEED command. Deliberately not called by
 * update_sv_state() or step-drill mode, which also write g_state.target_rpm
 * but are the machine talking to itself.
 */
...
void settings_set_depth_completion_action(uint8_t action);
void settings_set_quill_completion_action(uint8_t action);
void settings_set_load_completion_action(uint8_t action);
void settings_set_load_slip_completion_action(uint8_t action);
...
void settings_set_peck_completion_action(uint8_t action);
void settings_set_peck_reverse_out_ms(uint16_t ms);
