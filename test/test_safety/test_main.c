...
/* NOT "reverse", despite the old names: a sustained reverse reports plain 34.
 * These are the same stopped/running states with the transient direction-change
 * bits also set — see GF_STATE_MASK in config.h. */
#define GF_MOTOR_STOPPED_XBITS  436
#define GF_MOTOR_RUNNING_XBITS  438
#define GF_STATE_MASK           0x22
#define GF_KNOWN_BITS           0x1B6
...
/* This mirror claimed to inline task_motor.c "exactly" and did not: it accepted
 * all four values while the firmware compared for equality against only 32/34.
 * The test passed while a real 438 fell through to "Unknown GF state" and left
 * motor_running stale on a turning spindle. Now it mirrors the masked logic. */
static void classify_gf(uint16_t flags, bool *known_good, bool *error_state) {
    uint16_t gf_state = flags & GF_STATE_MASK;
    *known_good  = ((flags & (uint16_t)~GF_KNOWN_BITS) == 0) &&
                   (gf_state == GF_MOTOR_STOPPED || gf_state == GF_MOTOR_RUNNING);
    *error_state = (flags & 0x4000) != 0;
}

/* Does the masked state say "running"? */
static bool gf_running(uint16_t flags) {
    return (flags & GF_STATE_MASK) == GF_MOTOR_RUNNING;
}
...
void test_gf_known_good_stopped_reverse(void) {
    bool kg, es;
    classify_gf(GF_MOTOR_STOPPED_XBITS, &kg, &es);
    TEST_ASSERT_TRUE(kg);
    TEST_ASSERT_FALSE(es);
}
...
void test_gf_known_good_running_reverse(void) {
    bool kg, es;
    classify_gf(GF_MOTOR_RUNNING_XBITS, &kg, &es);
    TEST_ASSERT_TRUE(kg);
    TEST_ASSERT_FALSE(es);
}
...
/* The masking fix is only safe because of the known-bits check. 0x1234 masks to
 * exactly GF_MOTOR_STOPPED, so a mask-only implementation would publish a
 * spliced frame as a confident "spindle stopped". These pin that down. */
void test_gf_garbage_that_masks_to_a_valid_state_is_rejected(void) {
    bool kg, es;
    TEST_ASSERT_EQUAL_UINT16(GF_MOTOR_STOPPED, (uint16_t)(0x1234 & GF_STATE_MASK));
    classify_gf(0x1234, &kg, &es);
    TEST_ASSERT_FALSE(kg);

    /* 0x00FF masks to RUNNING and must also be rejected (bits 0,3,6 unknown). */
    TEST_ASSERT_EQUAL_UINT16(GF_MOTOR_RUNNING, (uint16_t)(0x00FF & GF_STATE_MASK));
    classify_gf(0x00FF, &kg, &es);
    TEST_ASSERT_FALSE(kg);
}

/* The transient-bit values must report the same running state as their plain
 * counterparts — that is the whole point of masking. */
void test_gf_transient_bits_report_same_running_state(void) {
    TEST_ASSERT_FALSE(gf_running(GF_MOTOR_STOPPED));
    TEST_ASSERT_FALSE(gf_running(GF_MOTOR_STOPPED_XBITS));
    TEST_ASSERT_TRUE(gf_running(GF_MOTOR_RUNNING));
    TEST_ASSERT_TRUE(gf_running(GF_MOTOR_RUNNING_XBITS));
}

/* An error value must not be laundered into a good state by the mask. */
void test_gf_error_bit_still_wins(void) {
    bool kg, es;
    classify_gf(16929, &kg, &es);
    TEST_ASSERT_TRUE(es);
    TEST_ASSERT_FALSE(kg);
}
...
int main(void) {
    ...
    RUN_TEST(test_gf_garbage_that_masks_to_a_valid_state_is_rejected);
    RUN_TEST(test_gf_transient_bits_report_same_running_state);
    RUN_TEST(test_gf_error_bit_still_wins);
