...
// Optional callback fired on every xTaskGetTickCount() — for dynamic mock state
typedef void (*mock_tick_callback_t)(TickType_t tick);
extern mock_tick_callback_t mock_tick_cb;
static inline TickType_t xTaskGetTickCount(void) {
    mock_tick_count += mock_tick_step;
    if (mock_tick_cb) mock_tick_cb(mock_tick_count);
