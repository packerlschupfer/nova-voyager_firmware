...
**Date:** 2026-01-30
**Status:** SUPERSEDED — graphics fully solved in lcd_research (2026-02/03)
**Update (2026-05):** GRAM addressing, CGRAM, coordinate system, and init
sequence are all fully understood and working. See `lcd_research/` project
for complete findings: `LCD_COMMAND_REFERENCE.md`, `SOLUTION.md`, `CHAR_CATALOG.md`.
Key corrections to this doc: SPI2 is irrelevant (graphics is 8-bit parallel only),
coordinate system IS standard ST7920, "vertical stripes" were XOR artifacts from
unfilled DDRAM, and CGRAM works (the test wrote to off-screen DDRAM address 0x80).
...
- **Addressing:** WORD-addressed — each DDRAM address holds TWO characters, so
  each row owns 8 addresses, not 16:
  Row 0: 0xC0-0xC7, Row 1: 0xD0-0xD7, Row 2: 0xC8-0xCF, Row 3: 0xD8-0xDF.
  `lcd_set_cursor(row, col)` takes a WORD index 0-7 (column N is character 2N).
  This line previously read "Row 0: 0xC0-0xCF …", i.e. one character per
  address — the reading that produced two real bugs (the CalcRPM screen
  printing onto row 3, and pong's win banner spilling onto row 2). See
  `include/lcd.h`.
