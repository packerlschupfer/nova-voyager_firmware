...
| 6 | BR | 0x4252 | 'B''R' | Brake/Spindle Hold (0/1) | Control | Medium | ❌ defined, never sent |
...
- **Usage** (**CORRECTED 2026-08-30 by measurement — see below**):
  - BR sets the brake *mode*, as `config.h` has always said ("Set brake mode").
    It is NOT an immediate powered hold.
  - The old text here — "BR(1) = Enable powered spindle hold (motor energized at
    zero speed)" — was **tested on the machine and is wrong**: with the spindle
    stopped and the guard closed, `BR=1` was sent alone, with no refresh, and
    left for 45 s. The chuck turned freely by hand and the MCB reported 0% load
    throughout. `BR=0` after. Repeated twice with the same result.
- **Response**: ACK
- **Effect**: Motor holds position against external torque when "stopped"
- **Timeout**: MCB auto-releases on its own (this doc said 30 s; the machine
  manual is reported as 20 s — not re-measured)
- **Nova**: ❌ **NOT USED.** Verified 2026-08-30: `motor_set_braking()` does not
  exist anywhere in the tree, and `CMD_SET_BRAKE` (0x4252) is defined in
  config.h and never sent. `CMD_MOTOR_BRAKE` in the command queue maps to
  `local_motor_stop_fast()`, not to BR. `power.braking_enabled` is a setting
  with no reader.
- **Why this mattered**: `src/spindle_hold.c` implements the powered hold as a
  nine-command VR/CL/VS sequence re-asserted every `HOLD_MAINTAIN_MS` (460 ms),
  and on this MCB that re-ACTUATES the brake rather than sustaining it — the
  operator watched it chatter at ~2 Hz. BR looked like the mechanism the machine
  was designed around. **It is not**: the test above showed no holding torque at
  all. The VR/CL/VS sequence really is the powered hold; the chatter has to be
  fixed inside it, not replaced by BR.
- **Lesson**: this file said "Nova: ✅ motor_set_braking(), called in
  local_motor_stop()" for a function that does not exist, and described a
  behaviour the command does not have. Treat its remaining unverified "✅"
  claims as unaudited.
