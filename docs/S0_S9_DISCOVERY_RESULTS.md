...
| **MR** | 02 21 15 | ❌ READER ARTIFACT, disregard | Later scans with a working reader return the complete frame `02 31 4D 52 30 03 1D` = **0**. In this same run LD/S2/S9 were truncated identically, and MR's `21` byte is the checksum of the PRECEDING GR reply — buffer misalignment, not MCB behaviour. |
