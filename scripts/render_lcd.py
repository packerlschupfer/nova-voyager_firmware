...
def load_bar_string(percent):
    """Generate 16-char load bar: 2-byte ■ (rendered as ##) for filled, spaces for empty."""
    if percent > 100:
        percent = 100
    filled = percent * 8 // 100  # 0..8 blocks
    return ("##" * filled).ljust(16)
...
# ---------------------------------------------------------------------------
# Screens — match current firmware display_update() layout
# ---------------------------------------------------------------------------
